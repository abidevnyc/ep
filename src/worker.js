...
async function trojanOverWSHandler(request, env) {
    ...
    readableWebSocketStream.pipeTo(new WritableStream({
        async write(chunk, controller) {
            ...
            handleTCPOutBound(remoteSocketWapper, addressRemote, portRemote, rawClientData, webSocket, log, env);
        },
        close() {
...
async function handleTCPOutBound(remoteSocket, addressRemote, portRemote, rawClientData, webSocket, log, env) {
    async function connectAndWrite(address, port) {
        const sniDomain = env.SNI || "example.com"; // 从环境变量中获取伪装域名
        const tcpSocket2 = connect({
            hostname: address,
            port,
            servername: sniDomain, // 添加 SNI 信息
        });
        remoteSocket.value = tcpSocket2;
        log(`connected to ${address}:${port} with SNI ${sniDomain}`);
